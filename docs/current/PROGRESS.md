...
### 계획 외 개선 — 외부 모델 코드 점검에서 나온 결함 수정

- 요청: 구현이 끝난 코드를 다른 모델(Opus 5)로 점검하고, 나온 결함까지 고칠 것.
- 조치:
  1. **이미지 읽기의 루트 경계 우회 차단** — `read_image`가 자체 경로 계산(`path.resolve` + 문자열 비교)을
     쓰고 있어 루트 안의 junction·심볼릭 링크로 바깥 파일을 읽을 수 있었다. 문서 읽기와 같은
     `resolveInsideRoot`(realpath 확인)를 타도록 고쳤다.
  2. **정화기 없는 상태를 안전한 쪽으로 닫음** — DOMPurify가 없을 때 정규식으로 흉내 내던 폴백을 없애고,
     정화기가 없으면 마크업을 넣지 않고 이스케이프한 원문만 표시하도록 했다(`sanitizer-missing`).
  3. **창 이동·새 창 요청 차단** — `will-navigate`와 `setWindowOpenHandler`를 걸어, 클릭 가로채기가 놓치는
     경로(Ctrl+클릭 등)에서도 앱 창이 외부 페이지로 가지 않고 http/https만 기본 브라우저로 넘어간다.
  4. **코드 뷰어의 읽던 자리 복원** — 껍데기가 비활성 탭을 `display:none`으로 감추면 스크롤이 초기화되는데,
     마크다운 뷰만 복원하고 코드 뷰는 복원하지 않았다. 스크롤 컨테이너를 기억해 `activate`에서 되돌린다.
     껍데기의 세션 상태 계약(`saveState`/`restoreState`)도 두 보기 모두 구현했다.
  5. **줄바꿈 처리를 동결본과 맞춤** — `breaks: true`라 소프트 줄바꿈이 전부 `<br>`이 되고 있었다.
     동결본(react-markdown + remark-gfm)과 같게 `breaks: false`로 고쳤다.
  6. **링크 대상 확장자별 분기** — 상대경로 링크를 확장자와 무관하게 마크다운 보기로 열고 있었다.
     `.md`는 마크다운, 그 밖의 지원 확장자는 코드 보기, 미지원 확장자는 열지 않는다.
  7. **렌더러 중복 등록 제거** — `marked.use()`를 렌더할 때마다 불러 누적되던 것을 최초 1회로 바꿨다.
  8. **차단된 링크 알림** — `console.warn`만 남기던 것을 화면 알림(`.view-notice`, 3초)으로 바꿨다.
  9. **UI 문자열 영어 통일** — 껍데기 메뉴가 영어인데 보기 쪽만 한국어였다(`복사` 등). 동결본의 영어 UI
     원칙에 맞춰 통일했다.
- 결과: 위 아홉 건이 모두 코드에 반영됐고, 아래 검증에서 재현하던 결함이 재현되지 않는다.
- 검증:
  1. junction으로 루트 밖을 가리키는 이미지·문서 읽기가 `ROOT_ESCAPE`로 거부되는 검사를
     `tests/test_phase_five.js`에 추가했다. 수정 전에는 이미지 경로로 바깥 파일 내용이 그대로 반환됐다.
  2. `tests/test_phase_three.js`가 정화기가 있을 때(정화기를 실제로 부르는지·금지 목록을 넘기는지)와
     없을 때(마크업을 넣지 않는지) 양쪽을 판정한다. 이전에는 정규식 폴백만 검사하고 있었다.
  3. `tests/test_phase_six.js`가 스텁이 아닌 실물 두 보기로 탭 전환 뒤 스크롤 복원과 세션 상태 계약을
     판정한다.
  4. `tests/test_phase_seven.js`의 회귀 대조에 렌더링 요소 13종과 줄바꿈 처리 대조를 더했고, 성능 측정은
     실제 파서·문법 강조·정화기를 태운 값(1MB 중앙값 233ms)으로 바꿨다. 폴백으로 측정되던 것을 막기 위해
     측정 전에 실제 파서가 붙었는지 먼저 판정한다.
  5. 기존 6개 테스트 파일 전건 통과, `shell/`·`shared/design/` 템플릿 대비 0줄 차이 유지.

### 계획 외 개선 — NFR-1의 기동 시간을 실제 앱으로 측정

- 요청: 위 점검에서 "실행부터 탐색기가 보일 때까지 3초"의 측정이 기준을 대표하지 못한다는 지적이 나왔다.
- 조치: Node에서 브릿지 함수만 부르던 값(0.9ms)은 참고값으로 이름을 바꾸고, `tests/measure_startup.js`를
  새로 만들어 Electron을 실제로 띄워 프로세스 시작부터 셸 로드 완료까지를 잰다. 호스트에는
  `MV_STARTUP_BENCH=1`일 때만 도는 측정 코드를 넣었다(껍데기는 건드리지 않았다).
- 결과: 실측 중앙값 **577ms**(5회: 679 · 557 · 587 · 570 · 577ms)로 3초 기준을 충족한다.
- 검증: `node tests/measure_startup.js 5` 실행. 창을 띄우지 못하면 실패로 끝나도록 했다.

### 확인이 필요한 것 — 마크다운 파일에도 걸린 2MB 상한

- `SPEC.md` FR-7과 D-8은 2MB 상한을 텍스트·코드 뷰어 기준으로 적었는데, 구현은 `.md`를 포함한 모든
  문서 읽기에 적용하고 있다. 상한이 없으면 큰 파일 하나로 앱이 멈출 수 있어 동작은 그대로 두었다.
- 요구를 바꾸려면 `SPEC.md`부터 고쳐야 하므로 여기 적어 두고 사람의 판단을 기다린다.
